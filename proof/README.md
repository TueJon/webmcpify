...
The harness is deliberately narrower than a full skill run: its phase narration,
before/after manifests, and tool implementation are prepared fixtures. It verifies
the approval-to-registration handoff and native Chrome behavior; it does not invoke
the skill's inventory, integration, heal, or audit phases.
...
- [`manifest.before.json`](manifest.before.json) is a prepared example of the
  inventory state at the human gate. The client mutation is still `discovered`.
- [`manifest.after.json`](manifest.after.json) is a prepared example of the expected
  final manifest, including approval, setup paths, and verification status. The
  runtime demo does not derive or assert these manifest files.
- [`integration.patch`](integration.patch) is an illustrative conceptual app diff.
  It abbreviates the handler body and is not a byte-for-byte patch generated by the
  harness.
