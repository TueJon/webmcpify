...
| Assertion compares object to string | Enumerated `inputSchema` may be stringified (native lag) or object (spec) — handle both | `typeof === 'string' ? JSON.parse : id` before comparing (see `verify.md`) |
